...
import { Sparkles, Plus, ShieldCheck, MapPin, Bike } from 'lucide-react-native';
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.trustStrip}>
        ...
        <View style={styles.trustDivider} />
        {/* « 0 Frais » n'est vrai qu'en phase 0 : en phase 1, on met en avant la livraison. */}
        {isPhase0 ? (
          <TrustItem tint={colors.status.infoLight} icon={<Sparkles size={14} color={colors.status.infoDark} />} title="0 Frais" sub="Garantis" />
        ) : (
          <TrustItem tint={colors.status.infoLight} icon={<Bike size={14} color={colors.status.infoDark} />} title="Livraison" sub="Par coursier" />
        )}
      </View>
    </View>
  );
